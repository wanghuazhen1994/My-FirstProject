    ...
    public class EnterGame : MonoBehaviour
    {
        ...
        private IEnumerator Start()
        {
            ...
            if (getUpdateInfoAsync.result == Request.Result.Success)
            {
                ...
                if (updateVersion.Major > playerVersion.Major || (updateVersion.Major == playerVersion.Major &&
                                                                  updateVersion.Minor > playerVersion.Minor))
                {
                    // 需要强更下载安装包。
                    ...
                    // Quit();
                    string _tipContent = string.Format(Constants.Text.TipsNewContent, updateVersion);
                    Debug.LogError("_tipContent == " + _tipContent);
                    yield break;
                }
                ...
                while (!getVersionsAsync.isDone)
                {
                    ...
                    Debug.LogError("progress ---"+getVersionsAsync.progress * 100 +"%");
                    yield return null;
                }
        ...
        private IEnumerator Reloading()
        {
            ...
            while (!reload.isDone)
            {
                ...
                Debug.LogError("reload.progress =="+reload.progress);
                yield return null;
            }
        ...
        private IEnumerator Downloading()
        {
            while (_downloadAsync.result != DownloadRequestBase.Result.Success)
            {
                ...
                Debug.LogError("_downloadAsync.progress ==" + _downloadAsync.progress);
                yield return null;
        ...
        private IEnumerator Clearing()
        {
            ...
            while (!removeAsync.isDone)
            {
                ...
                Debug.LogError("removeAsync.progress =="+removeAsync.progress);
                yield return null;
            }
        }
